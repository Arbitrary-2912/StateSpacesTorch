...
from s4torch.s42d.block2d import S42DBlock
...
if __name__ == "__main__":
    ...
    print(f"S4Model Params: {count_parameters(s42dmodel):,}")

    loss = nn.MSELoss()
    target = torch.randn_like(u)
    optimizer = torch.optim.Adam(s42dmodel.parameters(), lr=0.02)
    ...
    assert s42dmodel(u).shape == (u.shape[0], *u.shape[1:-1], s42dmodel.d_output)

...
class S4Layer(nn.Module):
    ...
    def K(self) -> torch.Tensor:  # noqa
        ...
        return conv.unsqueeze(0)  # [batch, features, seq_len]
    ...
    def forward(self, u: torch.Tensor) -> torch.Tensor:
        ...
        """
        return _non_circular_convolution(u, K=self.K) # + (self.D * u)  # Skip connection LTI


if __name__ == "__main__":
    N = 32          # dimensionality of states
    d_model = 128   # number of internal features
    l_max = 784     # 28 * 28 pixels presented sequentially

    u = torch.randn(4, l_max, d_model)

    s4layer = S4Layer(d_model, n=N, l_max=l_max).to(device=u.device)

    loss = nn.MSELoss()
    target = torch.randn_like(u)
    optimizer = torch.optim.Adam(s4layer.parameters(), lr=0.05)
    ...
    assert s4layer(u).shape == u.shape  # [batch_size, input_size, features] = [1, 784, 128] -> [1, 784, 128] (like attention)

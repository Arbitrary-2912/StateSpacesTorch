...
from s4torch.s43d.layer3d import S43DLayer
...
class S43DBlock(nn.Module):
    ...
    def __init__(
        ...
        self.pipeline = SequentialWithResidual(
            ...
            S43DLayer(d_model, n=n, seq_shape=seq_shape),
            activation(),
...
if __name__ == "__main__":
    ...
    u = torch.randn(1, *seq_shape, d_model)

    s43dblock = S43DBlock(d_model, n=N, seq_shape=seq_shape, norm_type="batch").to(device=u.device)
    print(f"S4Block Params: {count_parameters(s43dblock):,}")

    loss = nn.MSELoss()
    target = torch.randn_like(u)
    optimizer = torch.optim.Adam(s43dblock.parameters(), lr=0.05)
    ...
    assert s43dblock(u).shape == u.shape
